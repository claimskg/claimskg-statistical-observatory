#old file to plot since 2016 by month
import pandas as pd
...
def getSteamGraphData(theme, df, labels):
    ...
    df_result = dSmall.groupby([theme, pd.Grouper( key='date_cr_t', freq='Q')])['id1'].size().reset_index(name='counts')
    # df_result = dSmall.groupby([theme, pd.Grouper( key='date_cr_t', freq='M')])['id1'].size().reset_index(name='counts')
   ...
   #refaire un filtre sur les années souhaitées
    ...
    # df_result['month'] = df_result['date_cr_t'].dt.strftime('%B %Y')
    df_period['month'] = df_period['date_cr_t'].dt.strftime('%B %Y')
    # print(df_result)
    ...
    df_format = df_period[['month', 'counts']]
    print(df_period)
    # print(df_format)
