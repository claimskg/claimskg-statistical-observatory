...
def create_scatter_themes_dates_month():
    ...
    def buildTrace(dr,theme):
        df_result = dr.groupby([theme, pd.Grouper(key='date_cr_t', freq='M')])['id1'].size().reset_index(name='counts')
        trace = go.Scatter(
    ...
    scatter_themes_month_JSON = json.dumps(data, cls=plotly.utils.PlotlyJSONEncoder)

    return scatter_themes_month_JSON
